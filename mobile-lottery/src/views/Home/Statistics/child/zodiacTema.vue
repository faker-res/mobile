<template>
  <div class="posiCode_box" v-if="headerKey=='06'" ref="posiCode">
    ...
    <div class="charts_main">
      <div
        id="main"
        v-if="active==0"
        :style="{width:'100%',height:'50rem',top:'-5rem',position:'relative',zIndex:'-99'}"
      ></div>
         <div
        id="main"
        v-if="active==1"
    ...
    <div class="bar_text" style="top: 40rem;">
      <p v-show="active==0">出现期数（所选期数范围内遗漏的期数）</p>
      <p v-show="active==1">出现期数（所选期数范围内出现的期数）</p>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    async getPosiList() {
      try {
        let res = await this.$api.statistics.boseTeMa({ count: this.selected });
        if (res && res.code == 200) {
